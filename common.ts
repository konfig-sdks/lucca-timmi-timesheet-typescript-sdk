...
import { readableStreamToString, LuccaTimmiTimesheetError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new LuccaTimmiTimesheetError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new LuccaTimmiTimesheetError(e, e.response?.data)
                }
                if (innerError instanceof LuccaTimmiTimesheetError) {
                    // Got "LuccaTimmiTimesheetError" from the above try block
                    throw innerError;
                }
